...
export async function registerAIRoutes(app: any) {
    ...
    app.post("/api/ai/solve", async (req: any, res: any) => {
        ...
        const solutions = {
            ...
            other: "Relationship growth requires patience. Focus on active listening and validating your partner's feelings."
        };

        const type = issue?.toLowerCase()?.includes('libido') ? 'libido_mismatch' :
            issue?.toLowerCase()?.includes('talk') || issue?.toLowerCase()?.includes('communication') ? 'communication' :
                issue?.toLowerCase()?.includes('distant') ? 'distance' : 'other';
